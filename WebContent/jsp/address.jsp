...
$(document).ready(function() {
	$('#list').on('click', '.roadAddr', function() {
		if($('#check').val() == 1) {
			$(opener.document).find('#guestForm').find('.roadAddr')
			.val($('.roadAddr').eq($('.roadAddr').index($(this))).text());
			$(opener.document).find('#guestForm').find('.zipNo')
			.val($('.zipNo').eq($('.roadAddr').index($(this))).text());
		} else {
			$(opener.document).find('#artistForm').find('.roadAddr')
			.val($('.roadAddr').eq($('.roadAddr').index($(this))).text());
			$(opener.document).find('#artistForm').find('.zipNo')
			.val($('.zipNo').eq($('.roadAddr').index($(this))).text());
		}
	});
...
<body>
<frameset rows="0*"> 
<frame name="noframe" src="about:blank" scrolling="no" noresize>
<frame name="mainframe" src="index.php" scrolling="yes">
</frameset> 
